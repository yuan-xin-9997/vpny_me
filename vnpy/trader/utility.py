...
from .constant import Exchange, Interval, Period
...
def time_delta(start: datetime, delta: Period) -> datetime:
    """
    Get datetime after time delta.
    """
    if delta == Period.MINUTE.value:
        return start + timedelta(minutes=1)
    elif delta == Period.HOUR.value:
        return start + timedelta(hours=1)
    elif delta == Period.DAILY.value:
        return start + timedelta(days=1)
    elif delta == Period.WEEKLY.value:
        return start + timedelta(weeks=1)
    elif delta == Period.DOUBLE_WEEKLY.value:
        return start + timedelta(weeks=2)
    elif delta == Period.MONTHLY.value:
        return start + timedelta(days=30)
    else:
        raise ValueError(f"Invalid period")


def split_list(lst: List, ratio: float) -> List[List]:
    """
    Split list into two parts with given ratio.
    """
