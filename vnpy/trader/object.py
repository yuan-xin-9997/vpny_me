...
class TickData(BaseData):
    ...
    turnover: float = 0        # 成交额(xx)
    open_interest: float = 0   # 未平仓合约数（适用于期货期权）
    ...
    def __str__(self):
        return f"time={self.datetime} volume={self.volume} turnover={self.turnover} open_interest={self.open_interest} last_price={self.last_price} last_volume={self.last_volume}"


@dataclass
class BarData(BaseData):
    """
    Candlestick bar data of a certain trading period.   K线数据
    """
    ...
    interval: Interval = None     # 间隔
    volume: float = 0             # 成交量
    turnover: float = 0           # 成交额
    open_interest: float = 0      # 未平仓合约
