...
class CtaTemplate(ABC):
    ...
    def __init__(
        ...
        self.pos: int = 0  # todo：持仓，策略初始化的时候会从交易系统更新持仓信息吗
    ...
    def buy(
        ...
        """
        Send buy order to open a long position. 发送买入订单以建立多头头寸。
        """
        # 打印入参
        print("发送buy买指令（建立多头）: ", price, volume, stop, lock, net)
    ...
    def sell(
        ...
        """
        Send sell order to close a long position. 发送卖出指令以关闭多头头寸。
        """
        print("发送sell卖指令（关闭多头）: ", price, volume, stop, lock, net)
        return self.send_order(
            Direction.SHORT,
            Offset.CLOSE,
            price,
    ...
    def short(
        ...
        """
        Send short order to open as short position. 发送空头指令以建立空头头寸。
        """
        print("发送short开空仓指令（建立空头）: ", price, volume, stop, lock, net)
        return self.send_order(
            Direction.SHORT,
            Offset.OPEN,
            price,
    ...
    def cover(
        ...
        """
        Send cover order to close a short position.发送平仓指令以关闭空头头寸。
        """
        print("发送cover平空仓指令（关闭空头）: ", price, volume, stop, lock, net)
        return self.send_order(
            Direction.LONG,
            Offset.CLOSE,
            price,
    ...
    def cancel_order(self, vt_orderid: str) -> None:
        ...
        """
        if self.trading:
            print(f"{datetime.now()} 发送取消所有订单指令: ")
    ...
    def load_bar(
        ...
        bars: list[BarData] = self.cta_engine.load_bar(  # 从引擎中加载前days的历史K线数据
            self.vt_symbol,
            days,
            interval,  # 默认为分钟
            callback,
        ...
        for bar in bars:  # 调用回调函数
            callback(bar)
        print("load data done (加载历史数据完毕).")
