...
class DbBarData(Model):
    """K线数据表映射对象
    定义了一个名为 DbBarData 的 Peewee 模型类，用于映射数据库中的 K 线数据表（也称为蜡烛图数据）
    继承自 peewee.Model 基类

    模型定义关联：DbBarData 是一个 Peewee 的 Model 类，其 Meta 类中已配置了关联的数据库表名（默认是类名的小写形式 dbbardata，除非显式指定 table_name）。
    """

    id: AutoField = AutoField()  # 自增主键字段

    symbol: str = CharField()  # 标识字段
    exchange: str = CharField()
    datetime: datetime = DateTimeField()
    interval: str = CharField()

    # DoubleField 保证足够的精度存储金融数据
    volume: float = DoubleField()          # 成交量
    turnover: float = DoubleField()        # 成交额
    open_interest: float = DoubleField()   # 持仓量 (期货常用)
    open_price: float = DoubleField()      # 开盘价
    high_price: float = DoubleField()      # 最高价
    low_price: float = DoubleField()       # 最低价
    close_price: float = DoubleField()     # 收盘价


    class Meta:
        database: PeeweeMySQLDatabase = db  # 指定该模型使用的数据库连接为 db (一个 PeeweeMySQLDatabase 实例)
        # 定义了一个复合唯一索引
        #
        # 包含四个字段：symbol, exchange, interval, datetime
        #
        # True 表示这是一个唯一索引，确保这四者的组合是唯一的
        #
        # 这种设计可以防止同一时间点的同一K线被重复插入
